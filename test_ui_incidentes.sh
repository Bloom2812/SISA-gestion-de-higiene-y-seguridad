...
grep -q 'id="invMetodo"' index.html || fail "Falta el selector de metodología causal"
grep -q "'Ishikawa 6M'" app.js || fail "Falta la metodología Ishikawa 6M"
grep -q "'Análisis de barreras'" app.js || fail "Falta el análisis de barreras"
grep -q 'analisis_metodo' app.js || fail "Falta el almacenamiento estructurado del método"
grep -q 'renderizarAnalisisMetodo' app.js || fail "Falta la visualización dinámica del análisis"
